 ...
 * - Cadastro (signUp) com email redirectTo configurado
 * - Login via Google OAuth
 * - Modal para apoiar com PIX (cópia da chave)
 * - Redireciona para /home se o usuário já estiver autenticado
 */
const LoginPage: React.FC = () => {
  const navigate = useNavigate();
  const { user } = useAuth(); // garante redirecionamento automático quando já logado
  ...
  // chave PIX (apenas exemplo)
  const pixKey = 'lunara_terapias@jim.com';

  // Se já autenticado, vai para /home
  useEffect(() => {
    if (user) {
      navigate('/home');
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (isLogin) {
        ...
        // Em singInWithPassword, supabase já cria a sessão; redirecionamos
        navigate('/home');
      } else {
        // SIGN UP
        const { error: signUpError } = await supabase.auth.signUp({
          ...
          password,
          options: {
            // garante retorno para o seu domínio (dev/prod)
      ...
      else setError('Erro ao processar a solicitação. Tente novamente.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const { error: oauthError } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          // deixar consistente com as redirect urls configuradas no Supabase
          redirectTo: `${window.location.origin}/`,
        },
      });

      if (oauthError) throw oauthError;
      // Redirecionamento é feito pelo Supabase; não precisa navegar aqui
    } catch (err) {
