...
const HomePage = () => {
  ...
  const { user } = useAuth(); // <-- Verifica se está logado
  const { posts, loading } = usePosts();
  const { isInstallable, install } = usePWAInstall(); // <-- PWA
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white shadow-sm sticky top-0 z-10">
        <div className="container mx-auto px-4 py-3 flex justify-between items-center">
          ...
          {user && ( // <-- Só mostra se estiver logado
            <button
      ...
      {isInstallable && user && (
        <button
          onClick={install}
          className="fixed bottom-4 right-4 bg-emerald-600 text-white p-3 rounded-full shadow-lg z-50 hover:bg-emerald-700 transition"
